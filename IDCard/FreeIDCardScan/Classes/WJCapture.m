...
@implementation WJCapture
{
    unsigned char* _buffer;
    WJIDInfo * _lastIdInfo;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
        ...
        [outPutSetting isEqualToNumber:[NSNumber numberWithInt:kCVPixelFormatType_420YpCbCr8BiPlanarFullRange]]  )
    {
        ...
        {
            WJIDInfo *idInfo = [self idCardRecognit:sampleBuffer];
            if (idInfo !=nil && [idInfo isOK])
            {
                idInfo.curImage = self.stillImage;
...
- (WJIDInfo *)idCardRecognit:(CMSampleBufferRef)sampleBuffer
{
    WJIDInfo *idInfo = nil;
...
//            NSLog(@"IDCardRecApi，ret[%d]", ret);
            ...
            if ([idInfo isOK])
            {
                ...
                self.stillImage = [[UIImage alloc] initWithCGImage:videoImage];
            }
        }
    }
#endif
